  ...
  return (
    <>
      <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-end sm:items-center justify-center p-0 sm:p-4 overflow-hidden">
        <div className="bg-zinc-950 border-t sm:border border-zinc-800 rounded-t-3xl sm:rounded-3xl max-w-lg w-full max-h-[92dvh] sm:max-h-[88vh] flex flex-col overflow-hidden shadow-2xl animate-in slide-in-from-bottom duration-300">
          ...
          {/* Modal Top Banner */}
          <div className="relative bg-zinc-900 h-28 sm:h-32 px-5 sm:px-6 pt-5 sm:pt-6 flex items-start justify-between shrink-0 border-b border-zinc-800">
            <button
              onClick={onClose}
          ...
          <div className="px-5 sm:px-6 pb-6 relative -mt-12 sm:-mt-14 overflow-y-auto overscroll-contain space-y-4">
            <div className="flex items-end justify-between">
              <div className="relative">
                <img
                  src={avatar}
                  alt={name}
                  className="w-20 h-20 sm:w-24 sm:h-24 rounded-2xl object-cover border-4 border-zinc-950 shadow-xl"
                />
                {isVerified && (
                  <div className="absolute -bottom-1 -right-1 bg-white text-black rounded-full p-1 border-2 border-zinc-950 shadow-md" title="هوية ورخصة موثقة">
                    <CheckCircle2 className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-black" />
                  </div>
                )}
              </div>

              <div className="text-left bg-zinc-900 px-3.5 py-1.5 sm:px-4 sm:py-2 rounded-2xl border border-zinc-800">
                <div className="flex items-center gap-1 text-white font-extrabold text-base sm:text-lg">
                  <Star className="w-4 h-4 sm:w-5 sm:h-5 fill-white text-white" />
                  <span>{rating}</span>
                </div>
                <span className="text-[10px] sm:text-xs text-zinc-400 font-medium">من 5 نجوم</span>
              </div>
            </div>

            <div>
              <h3 className="text-xl sm:text-2xl font-black text-white mb-1">{name}</h3>
              <p className="text-xs sm:text-sm text-zinc-300 font-semibold flex items-center gap-1.5">
            ...
            <div className="bg-black p-3.5 rounded-2xl border border-zinc-800 space-y-2.5">
              ...
              {vehiclePhotos.length > 0 ? (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5">
                  {vehiclePhotos.map((photo, pIdx) => (
                    <div 
                      ...
                      className="relative rounded-2xl overflow-hidden border-2 border-zinc-800 hover:border-white aspect-video bg-zinc-900 group cursor-pointer shadow-md transition-all active:scale-95"
                      title="اضغط للتكبير"
                    >
                      <img
                        src={photo}
                      ...
                      {pIdx === 0 && (
                        <span className="absolute top-1.5 right-1.5 bg-black/90 text-white text-[9px] font-black px-2 py-0.5 rounded-md border border-zinc-700">
                          الرئيسية
                        </span>
                      )}
                    </div>
                ...
                <div className="bg-zinc-900/60 p-4 rounded-xl text-center text-xs text-zinc-400">
          ...
          <div className="bg-black p-3.5 rounded-2xl border border-zinc-800 space-y-2 mb-3.5 text-xs">
            <div className="text-[11px] font-bold text-zinc-400">أرقام التواصل المباشرة المسجلة:</div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <div className="flex items-center justify-between bg-zinc-900 border border-zinc-800 px-3 py-2 rounded-xl text-white">
          ...
          <p className="text-zinc-300 text-xs sm:text-sm bg-black p-3.5 sm:p-4 rounded-2xl border border-zinc-800 mb-4 sm:mb-6 leading-relaxed">
            "{bio}"
          </p>
          ...
          <div className="grid grid-cols-2 gap-2.5 sm:gap-3 mb-5 sm:mb-6">
            <div className="bg-zinc-900 p-3 rounded-xl border border-zinc-800">
              <div className="text-[11px] sm:text-xs text-zinc-400 mb-1 flex items-center gap-1">
            ...
            <div className="bg-zinc-900 p-3 rounded-xl border border-zinc-800">
              ...
              <div className="text-xs sm:text-sm font-bold text-white">هوية ورخصة مفعلة ✓</div>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2.5 sm:gap-3">
            {onSelectDriver && (
              <button
                ...
                className="flex-1 bg-white hover:bg-zinc-200 text-black font-black py-2.5 sm:py-3 px-4 rounded-xl shadow-lg transition-all text-center text-xs sm:text-sm active:scale-95"
              >
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              <a
                ...
                className="flex-1 sm:flex-initial flex items-center justify-center gap-1.5 bg-zinc-900 hover:bg-zinc-800 text-white font-black py-2.5 sm:py-3 px-4 rounded-xl border border-zinc-700 transition-colors text-xs sm:text-sm active:scale-95 shadow-md"
              ...
              <a
                href={`tel:${cleanCall}`}
                className="flex-1 sm:flex-initial flex items-center justify-center gap-1.5 bg-zinc-900 hover:bg-zinc-800 text-white font-bold py-2.5 sm:py-3 px-4 rounded-xl border border-zinc-700 transition-colors text-xs sm:text-sm active:scale-95 shadow-md"
    ...
    {selectedPhotoModal && (
      <div 
        ...
        className="fixed inset-0 z-60 bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 cursor-zoom-out animate-in fade-in"
      >
        <div className="relative max-w-4xl w-full max-h-[90vh] flex flex-col items-center">
          <button
            onClick={() => setSelectedPhotoModal(null)}
            className="absolute top-2 right-2 sm:top-4 sm:right-4 z-10 w-10 h-10 rounded-full bg-black/80 hover:bg-black text-white flex items-center justify-center border border-zinc-700 transition-all active:scale-95 shadow-2xl"
          >
            <X className="w-5 h-5" />
          </button>
          <img
            ...
            className="max-h-[85vh] w-auto max-w-full rounded-3xl object-contain border-2 border-zinc-700 shadow-2xl"
          />
        </div>
      </div>
    )}
