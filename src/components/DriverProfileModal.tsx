    ...
    <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-end sm:items-center justify-center p-0 sm:p-4 overflow-hidden">
      <div className="bg-zinc-950 border-t sm:border border-zinc-800 rounded-t-3xl sm:rounded-3xl max-w-lg w-full max-h-[92dvh] sm:max-h-[88vh] flex flex-col overflow-hidden shadow-2xl animate-in slide-in-from-bottom duration-300">
        ...
        <div className="px-5 sm:px-6 pb-6 relative -mt-12 sm:-mt-14 overflow-y-auto overscroll-contain">
          ...
          <p className="text-xs sm:text-sm text-zinc-300 font-semibold mb-2.5 flex items-center gap-1">
          ...
          {vehiclePhotos.length > 0 && (
            <div className="mb-3.5 bg-black p-2.5 rounded-2xl border border-zinc-800 space-y-2">
              <div className="flex items-center justify-between text-[11px] font-bold text-zinc-300">
                <span>صور المركبة المعتمدة للتوصيل:</span>
                <span className="text-[10px] text-zinc-400 font-normal">{vehiclePhotos.length} {vehiclePhotos.length === 1 ? 'صورة' : 'صور'}</span>
              </div>
              <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                {vehiclePhotos.map((photo, pIdx) => (
                  <div key={pIdx} className="relative rounded-xl overflow-hidden border border-zinc-700 aspect-video bg-zinc-900 group">
                    <img
                      src={photo}
                      alt={`Vehicle ${pIdx + 1}`}
                    ...
                    />
                    {pIdx === 0 && (
                      <span className="absolute top-1 right-1 bg-black/90 text-white text-[8px] font-black px-1 rounded border border-zinc-700">
                        الرئيسية
                      </span>
                    )}
                  </div>
