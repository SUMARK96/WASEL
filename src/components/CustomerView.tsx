    ...
    <div className="space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
          <div>
            ...
            <h1 className="text-2xl md:text-3xl font-black text-white mb-2">
              أنشئ طلبك واحصل على أفضل عروض السائقين المعتمدين
            </h1>
            <p className="text-slate-400 text-sm max-w-xl leading-relaxed">
              انشر طلب توصيل طردك بين أي إمارتين في الدولة، واستقبل عروض الأسعار مرتبة بالأعلى تقييماً وجودة مع تواصل واتساب ومكالمة فوري.
            </p>
          </div>
      ...
      {filteredRequests.length === 0 ? (
        <div className="bg-slate-900/40 rounded-3xl p-12 text-center border border-slate-800/80">
          ...
          <p className="text-slate-400 text-xs mb-6">قم بنشر طلب توصيل طرد جديد للاستفادة من شبكة السائقين المعتمدين.</p>
          <button
            onClick={onOpenNewRequest}
                  ...
                  <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-2xl p-4 flex flex-col lg:flex-row items-center justify-between gap-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2.5 w-full lg:w-auto">
                      ...
                      <a
                        ...
                        className="flex-1 lg:flex-initial flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black px-4 sm:px-5 py-3 rounded-xl text-xs transition-all shadow-lg shadow-emerald-500/20 transform active:scale-95"
                      ...
                      {onOpenRateDriver && (
                        req.isCustomerRated ? (
                          <div className="flex items-center gap-1.5 bg-amber-500/15 border border-amber-500/30 text-amber-300 font-bold px-3 py-3 rounded-xl text-xs">
                            <CheckCircle2 className="w-4 h-4 text-emerald-400" />
                            <span>تم تقييم السائق ({req.customerRating} ⭐)</span>
                          </div>
                        ) : (
                          <button
                            onClick={() => onOpenRateDriver(req, acceptedOffer)}
                            className="flex-1 lg:flex-initial flex items-center justify-center gap-1.5 bg-amber-500 hover:bg-amber-400 text-slate-950 font-black px-4 py-3 rounded-xl text-xs transition-all shadow-lg shadow-amber-500/20 transform active:scale-95 animate-pulse"
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {sortedOffers.map((offer, index) => {
                          ...
                          return (
                            <div
                              key={offer.id}
                              className={`bg-slate-950 rounded-2xl p-4 transition-all flex flex-col justify-between space-y-4 relative ${
                                isTopRanked
                            ...
                            >
                              ...
                              {isTopRanked && (
                                <div className="absolute -top-3 right-4 bg-gradient-to-r from-amber-500 via-amber-400 to-amber-500 text-slate-950 text-[10px] font-black px-3 py-0.5 rounded-full shadow-md flex items-center gap-1">
                                ...
                                <div className="flex items-center gap-3">
                                  <img
                                    ...
                                    className={`w-12 h-12 rounded-xl object-cover border ${
                                      isTopRanked ? 'border-amber-400 ring-2 ring-amber-500/30' : 'border-slate-800'
                                    }`}
                                  />
                                  <div>
                                    ...
                                    <div className="flex items-center gap-2 text-xs text-slate-400 mt-0.5">
                                      ...
                                      <span>({offer.driverCompletedCount} توصيلة ناجحة)</span>
                                    </div>
                                  </div>
                                </div>
                              ...
                              <div className="flex gap-2">
                                ...
                                <button
                                  ...
                                  className="flex-1 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-slate-950 font-black py-2.5 px-3 rounded-xl text-xs transition-all shadow-md shadow-amber-500/10 transform active:scale-95"
                                >
                                  قبول العرض
                                </button>
                              </div>
