      ...
      <div className="bg-slate-900 border-t sm:border border-slate-800 rounded-t-3xl sm:rounded-3xl max-w-xl w-full max-h-[92dvh] sm:max-h-[88vh] flex flex-col overflow-hidden shadow-2xl animate-in slide-in-from-bottom duration-300">
        ...
        <div className="bg-slate-950 px-4 sm:px-6 py-3.5 sm:py-4 border-b border-slate-800 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm sm:text-lg font-black text-white">الاشتراك الشهري الموحد للسائقين</h3>
              <p className="text-[11px] sm:text-xs text-slate-400">باقة موحدة لجميع كباتن واصل • عمولة 0%</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-5 overscroll-contain">
          {paymentDone ? (
            <div className="p-6 sm:p-8 text-center flex flex-col items-center justify-center space-y-4">
              <div className="w-16 h-16 sm:w-20 sm:h-20 bg-emerald-500/20 text-emerald-400 rounded-full flex items-center justify-center border-2 border-emerald-500/30 animate-bounce">
                <ShieldCheck className="w-8 h-8 sm:w-10 sm:h-10" />
              </div>
              <h4 className="text-lg sm:text-2xl font-black text-white">تم تجديد اشتراكك بنجاح! 🎉</h4>
              <p className="text-slate-300 text-xs sm:text-sm max-w-md leading-relaxed">
                حسابك الآن نشط على <span className="text-cyan-400 font-bold">{plan.name}</span>. يمكنك الاستمرار بتقديم العروض للعملاء وتلقي الإشعارات الفورية.
              </p>
            </div>
          ) : (
            <>
              ...
              <form onSubmit={handlePay} className="bg-slate-950/80 p-4 sm:p-5 rounded-2xl border border-slate-800 space-y-3.5 sm:space-y-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-400 mb-1">رقم البطاقة</label>
                  <input
                    type="text"
                    value={cardNumber}
                ...
                <button
                  ...
                  className="w-full bg-gradient-to-r from-blue-600 via-blue-500 to-cyan-500 hover:from-blue-500 hover:to-cyan-400 text-white font-black py-3 sm:py-3.5 rounded-xl shadow-xl shadow-blue-500/25 transition-all text-xs sm:text-sm flex items-center justify-center gap-2 disabled:opacity-50 active:scale-95"
                >
                  {isProcessing ? (
                    <>
